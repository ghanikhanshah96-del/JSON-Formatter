...
import { homeSchemas } from "@codeformattools/seo";
...
export default function Home() {
  return <main>
    ...
    <section className="hero"><div className="container hero-grid"><div>
      <div className="eyebrow"><span className="live-dot" /> BROWSER FIRST DEVELOPER TOOLS</div>
      <h1>Make sense of<br /><em>your data.</em></h1>
      <p className="hero-copy">Format, validate, and convert structured data in seconds. No upload, no account, no distractions.</p>
      ...
      <div className="hero-proof"><span>✦ Private by design</span><span>⚡ Runs in your browser</span><span>◇ Free to use</span></div>
    ...
    <section className="tools-section container" id="tools"><div className="section-heading"><div><div className="eyebrow">THE TOOLBOX</div><h2>Everything you need.<br /><span>Nothing you don’t.</span></h2></div><p>A focused set of tools built for the work you do every day. Format JSON, SQL, YAML, and XML, or convert between structured formats, locally.</p></div><ToolsGrid tools={tools} categories={categories} /></section>
    <section className="privacy-band"><div className="container privacy-grid"><div className="privacy-symbol" aria-hidden="true">🛡</div><div><div className="eyebrow">PRIVACY IS THE DEFAULT</div><h2>Your data stays<br /><em>on your device.</em></h2><p>Everything runs in your browser. Your input is never uploaded, logged, or stored by Code Format Tools. Work with real data confidently.</p><Link href="/privacy">How our privacy works <span aria-hidden="true">→</span></Link></div><div className="privacy-list"><div><span>01</span><strong>Local processing</strong><p>Every operation runs in a dedicated browser worker.</p></div><div><span>02</span><strong>No account required</strong><p>Open a tool and get straight to work.</p></div><div><span>03</span><strong>Input never stored</strong><p>We do not save your pasted text or files.</p></div></div></div></section>
    <section className="bottom-cta container"><div className="eyebrow">READY WHEN YOU ARE</div><h2>Less friction.<br /><em>More flow.</em></h2><Link className="button primary" href="/json-formatter">Start formatting <span aria-hidden="true">→</span></Link></section>
  </main>;
}
